...
app.get("/api/friends/:publicAddress", async (req, res) => {
  try {
...
app.get("/", (req, res) => {
  res.json({
    ...
    endpoints: {
      "POST /api/will/prepare": "Prepare a will with nominees and deadline",
      "GET /api/friends/:publicAddress": "Get friends list by public address",
      "POST /api/friends": "Add a new friend",
      "POST /api/friends/connect": "Connect two users as friends",
...
app.listen(PORT, () => {
  ...
  console.log(
    `Will prepare endpoint: http://localhost:${PORT}/api/will/prepare`
  );
});
